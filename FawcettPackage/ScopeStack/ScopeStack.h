...
// Application:   Application:  Project3, CSE687 - OOD Spring 2019         //
...
/*
  ...
  Maintenance History:
  ====================
  ver 2.3 : 5 March 2019
  - add new function myTreeWalk()
  ver 2.2 : 29 Oct 2016
...
namespace CodeAnalysis
{
  ...
  //----< walk tree of element nodes and store class and function information into table >---------------------------------
  template<typename element>
  void myTreeWalk(element* pItem, std::unordered_map<int, std::string> &temp)
  {
	  pItem->findClassFunction(temp);
	  auto iter = pItem->children_.begin();
	  while (iter != pItem->children_.end())
	  {
		  myTreeWalk(*iter, temp);
		  ++iter;
	  }
  }
}
