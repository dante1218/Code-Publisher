...
* ----------------------
*  ver 1.1 : 4/30/2019
*  - Add several message  functions to support Code Publisher
*	- getConvertedFiles(): receive command from client and send all converted files' name to client 
*	- publishDirs(): receive command from client and convert all required file by calling function from Code Publisher
*   - downloadFile(): receive command from client and send desired file (real file, not name) to client
*  ver 1.0 : 3/27/2018
*  - first release
*/
#include <vector>
...
#include "../PublisherFactory/PublisherFactory.h"
#include <windows.h>
#include <tchar.h>
...
namespace Repository
{
	...
	using ConvertedFilePath = std::string;
	using Key = std::string;
	using Msg = MsgPassingCommunication::Message;
	using ServerProc = std::function<Msg(Msg)>;
	using MsgDispatcher = std::unordered_map<Key, ServerProc>;

	const SearchPath storageRoot = ".."; 
	const ConvertedFilePath pathName = "../convertedPages";
